...
This tutorial provides the source code for the New York State Wastewater Surveillance Network's SARS-CoV-2 data dashboard that was retired in 2024. You can now view a test dashboard built for this tutorial at [this link](https://nywastewatcher.shinyapps.io/wbe-dashboard-example/). The tutorial goes through the major code chunks for processing the wastewater data, case data, and spatial data used in the R shiny application. Further, the tutorial discusses some of the techniques used to provide automatic data updates to the dashboard, how each figure was created, and how interactivity was coded into the dashboard. Portions of the source code are provided in this document. This tutorial represents the state of the dashboard on the date of publication and may not reflect the most up-to-date version of the code or dashboard.

#### Please reference the following paper when using this resource: 

Hill D, Dunham C, Larsen DA, Collins M. Operationalizing an open-source dashboard for communicating results of wastewater-based surveillance. MethodsX. 2023 Dec 1;11:102299. [https://doi.org/10.1016/j.mex.2023.102299](https://doi.org/10.1016/j.mex.2023.102299)
...
## Adding additional pathogens

The original dashboard for the NY Wastewater Surveillance Network was designed for SARS-CoV-2. With programs adding pathogens beyond SARS-CoV-2/COVID-19, it is important to include the possibility for displaying multiple disease or targets in one dashbaord. Adding additional targets to your data dashboard can be done in several ways. Our example tutorial dashboard does this by adding a `selectInput` dropdown menu for toggling the map and graphs between targets. 

### Example data for additional pathogens
The example dashboard creates a dummy dataset with two pathogens using the code below.
...
### Placement of `selectInput`
The `selectInput` is added to the dashboard sidebar.

![Example placement of dropdown](E:/Dropbox/CEMI/Wastewater/Papers/Dashboard_paper/Figures and Tables/sidebar with pathogen selector.png)
You can name the options according to each pathogen.
...
### Link the dropdown to the map and graphs

Switching the map and graphs requires a few extra chunks of code.

For switching the maps, you will need to make the spatial data that is mapped reactive using the `reactive` command from RShiny.
...
You will then call the reactive object in your leaflet code. Reactive objects are similar to functions and thus need open parentheses after their name. For example, our reactive object will be called using `ny_pathogen_filtered()`.
...
Since the spatial object is now a reactive object, we also need to wrap the leafletProxy in an `observe` element to activate the reactive componenet. We will generate the color factors here so they change when the new pathogen is selected.
...
### Additional pathogen graphs

For the graphs, we will also create a reactive object and call the object when we render the plots. You do not need an additional `observe` command for the plot since the `render` functions are already coded for interactivity.
